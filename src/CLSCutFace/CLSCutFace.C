/*---------------------------------------------------------------------------*\
              ...
              Modified work | Copyright (C) 2018      Linfan Zhang
-------------------------------------------------------------------------------
...
int Foam::CLSCutFace::debug = 0;

// * * * * * * * * * * * * * * * * Constructors  * * * * * * * * * * * * * * //

Foam::CLSCutFace::CLSCutFace
(
    const fvMesh& mesh,
...
void Foam::CLSCutFace::calcSubFaceCentreAndArea()
{
...
Foam::label Foam::CLSCutFace::calcSubFace
(
...
//    label pl1 = pLabels[0];
    label pl1 = 0;
    scalar f1 = f[pl1];
    // If vertex values are very close to CLSValue lift them slightly to avoid
    ...
    if (mag(f1 - CLSValue) < 10*SMALL)
    {
        f1 += sign(f1 - CLSValue)*10*SMALL;
    }
...
//        label pl2 = pLabels[pLabels.fcIndex(pi)];
        label pl2 = pLabels.fcIndex(pi);
        scalar f2 = f[pl2];
//        scalar f2 = f[pi+1];
        if (mag(f2 - CLSValue) < 10*SMALL)
        {
            f2 += sign(f2 - CLSValue)*10*SMALL;
        }
        if (f1 > CLSValue)
        {
            ...
            if (f2 < CLSValue)
            {
                lastEdgeCut_ = (CLSValue - f1)/(f2 - f1);
            }
        }
        else if (f1 < CLSValue && f2 > CLSValue)
        {
            if (firstFullySubmergedPoint_ == -1)
            {
                firstFullySubmergedPoint_ = pLabels.fcIndex(pi);
                firstEdgeCut_ = (CLSValue - f1)/(f2 - f1);
            }
            else
            {
                if (debug)
                {
                    ...
                    Pout<< "Face values: f-CLSValue = " << endl;
                    forAll(fl, fpi)
                    {
                        Pout<< f[fpi] - CLSValue << " ";
//                        Pout<< f[fl[fpi]] - CLSValue << " ";
                    }
                    Pout<< " " << endl;
                }
            }
    ...
    {
        // Face entirely above CLSsurface
        faceStatus = 1;
    }
    else
    {
        // Face entirely below CLSsurface
        faceStatus = -1;
    }
    return faceStatus;
}
...
void Foam::CLSCutFace::subFacePoints
(
    const pointField& points,
    const labelList& pLabels
)
...
void Foam::CLSCutFace::surfacePoints
(
    const pointField& points,
    const labelList& pLabels
)
...
)
{
    ...
    const scalar& a = nHat_[celli].x();
    const scalar& b = nHat_[celli].y();
    const scalar& c = nHat_[celli].z();
    scalar d0 =  a*CC.x() + b*CC.y() + c*CC.z();

    forAll(pLabels,pi)
    {
        const vector& pp = points[pLabels[pi]];
        fvert[pi] = (a*pp.x() +b*pp.y() +c*pp.z() - d0);
    }
    return calcSubFace(CLSValue, points, fvert, pLabels);
}
...
)
{
    ...
    return calcSubFace(CLSValue, points, f, pLabels);
}


const Foam::point& Foam::CLSCutFace::subFaceCentre()
{
...
const Foam::vector& Foam::CLSCutFace::subFaceArea()
{
    if (!subFaceCentreAndAreaIsCalculated_)
    {
        calcSubFaceCentreAndArea();
...
const Foam::DynamicList<Foam::point>& Foam::CLSCutFace::subFacePoints() const
{
    return subFacePoints_;
}


const Foam::DynamicList<Foam::point>& Foam::CLSCutFace::surfacePoints() const
{
    return surfacePoints_;
}


void Foam::CLSCutFace::clearStorage()
{
    firstEdgeCut_ = -1;
    lastEdgeCut_ = -1;
    firstFullySubmergedPoint_ = -1;
...
Foam::scalar Foam::CLSCutFace::timeIntegratedArea
(
...
void Foam::CLSCutFace::cutPoints
(
...
void Foam::CLSCutFace::quadAreaCoeffs
(
